 ...
 */
public class FriendOldService extends Service implements RosterListener {
    ...
    private void saveFriendToDB(final String request_jid) {
        new Thread(new Runnable() {
            ...
            public void run() {
                //自己的信息
                mUser = CommonUtil.getUserInfo(FriendOldService.this);
...
//                _openfire_username = "165094350";//test
                                ...
                                if (_success) {
                                    ...
                                    if (_user_json_str != null && !_user_json_str.equals("")) {
                                        ...
                                        Friend _friend = dbManager.selector(Friend.class)
                                                ...
                                                .findFirst();
                                        if (_friend == null) {
                                            Log.d("jlj", "FriendService1-------------------------friend add");
                                            //新增该好友信息到数据库
                                            _friend = new Friend();
                                            ...
                                            Log.d("jlj", "FriendService1-------------------------friend update");
                                            //更新好友信息
                                            _friend.setStatus(2);//待接收
    ...
    private void actionToNotifyNewFriendActivity(String _friend_json) {
        ...
        boolean isForeground_MainActivity = CommonUtil.isForeground(FriendOldService.this, "com.mkch.youshi.MainActivity");

        //发送广播：若UI处于显示状态，则通知界面更新UI；若UI不处于显示状态，弹出通知栏，显示信息条数和最新的信息。
        boolean isForeground = CommonUtil.isForeground(FriendOldService.this, "com.mkch.youshi.activity.NewFriendActivity");
...
//                .setFullScreenIntent(pendingIntent,true);
    ...
    private void actionToNotifyChatActivity(ChatBean _chat_bean, int chat_id, Friend _friend) {
        //发送广播：若UI处于显示状态，则通知界面更新UI；若UI不处于显示状态，弹出通知栏，显示信息条数和最新的信息。
        boolean isForeground = CommonUtil.isForeground(FriendOldService.this, "com.mkch.youshi.activity.ChatActivity");
        if (isForeground) {
